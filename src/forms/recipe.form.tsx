...
const RecipeForm = ({ initialRecipe }: RecipeFormProps) => {
  ...
  const handleRemoveIngredientFilds = (id: number) => {
  ...
  return (
    <Form className="w-[450px]" action={handleSubmit}>
      ...
      <Input
        ...
        validate={(value) => (!value ? "Обязательный параметр!" : null)}
      />

      <Input
        name="description"
        placeholder="Введите описание (необязательный параметр)"
        type="text"
      ...
      <Input
        name="imageUrl"
        placeholder="Введите url изображения (необязательный параметр)"
        type="url"
      ...
      <div className="space-y-2 w-full">
          ...
          <div className="flex gap-2 items-center" key={field.id}>
            ...
            {ingredientFields.length > 1 && (
              <Button
                color="danger"
                variant="light"
                onPress={() => handleRemoveIngredientFilds(field.id)}
                className="w-[30px]"
              >
                -
              </Button>
        ...
        {ingredientFields.length < 10 && (
          <Button
            color="primary"
            variant="flat"
            onPress={handleAddIngredientFields}
          >
            +
          </Button>
        )}
